...
tf.app.flags.DEFINE_integer('resnet_size', 4, 'n: the size of ResNet-(6n+2)')
...
def model(images, is_training=True):
    ...
    outputs_merge[0] = tf.layers.conv2d(tf.concat([outputs[2],unpool(outputs[3])], axis=-1), 512, 1, 1, "same")
    outputs_merge[0] = batch_norm_relu(outputs_merge[0], is_training)
    outputs_merge[0] = tf.layers.conv2d(outputs_merge[0], 512, 3, 1, "same")
    outputs_merge[0] = batch_norm_relu(outputs_merge[0], is_training)

    outputs_merge[1] = tf.layers.conv2d(tf.concat([outputs[1], unpool(outputs_merge[0])], axis=-1), 256, 1, 1, "same")
    outputs_merge[1] = batch_norm_relu(outputs_merge[1], is_training)
    outputs_merge[1] = tf.layers.conv2d(outputs_merge[1], 256, 3, 1, "same")
    outputs_merge[1] = batch_norm_relu(outputs_merge[1], is_training)

    outputs_merge[2] = tf.layers.conv2d(tf.concat([outputs[0], outputs_merge[1]], axis=-1), 128, 1, 1, "same")
    outputs_merge[2] = batch_norm_relu(outputs_merge[2], is_training)
    outputs_merge[2] = tf.layers.conv2d(outputs_merge[2], 128, 3, 1, "same")
    outputs_merge[2] = batch_norm_relu(outputs_merge[2], is_training)
    for i in range(3):
...
def start_layer(inputs, training):
    ...
    outputs = batch_norm_relu(outputs, training)
    outputs = tf.layers.conv2d(outputs, 128, 3, 1, "same")
